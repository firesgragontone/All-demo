...
public class BlogDemo {
    ...
    public long getContentId() {
        return jedis.incr("content_id_counter");
    }
    ...
    public boolean publishContent(long id, Map<String, String> content) {
        if (jedis.hexists("aritcle::" + id, "title")) {
            return false;
        }
        content.put("content_len", String.valueOf(findBlogById(id).get("content").length()));
        jedis.hmset("article::" + id, content);
        return true;
    }
    ...
    public Map<String,String> getContentById(long id){
        Map<String,String> content = jedis.hgetAll("article::"+id);
        incrementContentViewCount(id);
        return content;
    }
    ...
    public void updateContent(long id,Map<String,String> updateContent){
        String content = updateContent.get("content");
        if(content != null && !"".equals(content)){
            updateContent.put("content_length",String.valueOf(content.length()));
        }
        jedis.hmset("article::"+id,updateContent);
    }
    ...
    public void incrementContentViewCount(long id){
        jedis.hincrBy("article::"+id,"view_count",1);
    }
